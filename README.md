...
- "**mandelF18A**"" This requires a F18A and runs completely in the GPU for super-fast rendering. It renders at half-horizontal resolution (fat pixels), so benchmark numbers are not comparable to the CPU version (***mandel99**), but can be used for F18A GPU speed comparisons.
